...
The **first** one is an _env_ file: when Docker runs the container, it will use this file to create several environment variables and when the server loads its configuration, it will read the values for its settings from these variables.\
...
The uppercase part of each variable matches its name as described in the [Setting up the server](#setting-up-the-server) section, while on the right is the value to assign.\
